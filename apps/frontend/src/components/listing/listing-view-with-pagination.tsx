...
import { IPropsWithPagination } from './types/listing';

const ListingViewWithPagination = <T, F extends IFilterOptions>({
  columns,
  useData,
  defaultPageSize = 10,
  filterValues,
  ...
  useEffect(() => {
    ...
    setData(() => {
      if (!(isLoading || isFetching)) {
        return record && record.results ? (record.results as object[]).map(r => ({ original: r })) : [];
      }
      return [];
    });
...
export default ListingViewWithPagination;
