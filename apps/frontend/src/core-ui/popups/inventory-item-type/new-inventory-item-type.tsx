...
import { InventoryType } from 'interfaces/ISettings';

interface IProps {
  update?: boolean;
  inventory_type?: InventoryType;
  createInventoryType?: GenericMutationTrigger<IIDName, InventoryType>;
  updateInventoryType?: GenericMutationTrigger<Partial<InventoryType>, InventoryType>;
}

const InventoryTypeSchema = Yup.object().shape({
  name: Yup.string().trim().required('This field is required!').min(1),
});

const NewInventoryItemTypeModal = ({
  update = false,
  createInventoryType,
  inventory_type,
  updateInventoryType,
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, setFieldError }) => {
      ...
      if (update && inventory_type && Number(inventory_type.id) > 0) {
        updateInventoryType &&
          updateInventoryType({ ...values, id: Number(inventory_type.id) })
            .then(result => {
              if (result.data) {
                SwalExtended.close();
              } else {
  ...
  return (
    <Popup
      ...
      subtitle={`Fill out the details to ${update ? 'update existing' : 'create new'} inventory item type`}
      onSubmit={handleSubmit}
      isSubmitting={isSubmitting}
      onReset={handleReset}
      successButton={update ? 'Update' : 'Add'}
    >
      <Form.Group className="mb-3" controlId="NewInventoryItemTitle">
        <Form.Label className="popup-form-labels">Title</Form.Label>
        <Form.Control
          autoFocus
          type="text"
          placeholder="Enter a name of an item type"
          name="name"
          value={values.name}
...
export default NewInventoryItemTypeModal;
