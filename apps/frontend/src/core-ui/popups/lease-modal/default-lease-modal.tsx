...
const DefaultLeaseTemplateSchema = Yup.object().shape({
  lease_template: yupFilterInput.required('This field is required!'),
  default_lease_agenda: Yup.string().trim(),
...
const DefaultLeaseModal = ({ property, files, lease_template }: IProps) => {
  const [updateLeaseTemplate] = useUpdatePropertiesInformationMutation();

  const [createAttachment] = useCreateDefaultLeaseTemplateAttachmentsMutation();
  const [deleteAttachment] = useDeleteDefaultLeaseTemplateAttachmentsMutation();
  ...
  const handleFormSubmission = async (values: Partial<ISingleProperty>) => {
    ...
    const response = await updateLeaseTemplate({ ...values, id: record_id }).unwrap();
  ...
  const handleAttachments = async (files: IFileInfo[], attachment__id: number) => {
    ...
    const promises: Array<
      GenericMutationResult<Partial<IPropertyAttachments>, 'PropertyLeaseAttachments', IPropertyAttachments>
    > = [];
  ...
  const handleDeleteOldAttachments = async (files: IPropertyAttachments[]) => {
    const promises: Array<GenericMutationResult<string | number, 'PropertyLeaseAttachments', void>> = [];
    if (files.length > 0) {
  ...
  const formik = useFormik({
    initialValues: {
      lease_template: lease_template ? [lease_template] : ([] as Option[]),
      default_lease_agenda: property?.default_lease_agenda ?? '',
      files_included: files.length > 0 ? files : [],
      files: [] as Array<File>,
    },
    validationSchema: DefaultLeaseTemplateSchema,
    enableReinitialize: true,
    onSubmit: (values, { setSubmitting, setFieldError }) => {
      setSubmitting(true);
      SwalExtended.showLoading();

      let lease_template_id = 0;
      if (values.lease_template && values.lease_template.length > 0) {
        lease_template_id = Number((values.lease_template as Array<ISingleLeaseTemplate>)[0].id);
      }

      const data: Partial<ISingleProperty> = {
        ...values,
        lease_template: lease_template_id,
        default_lease_template: lease_template_id,
      };
  ...
  const onLeaseTemplateSelected = useCallback(
    (selected: Option[]) => {
      if (selected.length) {
        setFieldValue('lease_template', selected);
      } else {
        setFieldValue('lease_template', []);
      }
    },
    [setFieldValue]
  );
  ...
  return (
    <Popup
      title={'Default Lease template'}
      subtitle={'Update lease template information'}
        ...
        <Col xxl={7} md={6}>
          <div className="text-start">
            <FilterPaginateInput
              ...
              selected={values.lease_template}
              onSelectChange={onLeaseTemplateSelected}
              onBlurChange={() => setFieldTouched('lease_template', true)}
              isValid={touched.lease_template && !errors.lease_template}
              isInvalid={touched.lease_template && !!errors.lease_template}
              model_label="lease.LeaseTemplate"
              labelKey={'name'}
              error={errors.lease_template}
            />
            <Form.Group controlId="DefaultLeaseFormAgenda">
              <Form.Label className="popup-form-labels">Agenda</Form.Label>
              <Form.Control
                placeholder="Some test here..."
                as="textarea"
                rows={5}
                name="default_lease_agenda"
                value={values.default_lease_agenda}
                onChange={handleChange}
                onBlur={handleBlur}
                isValid={touched.default_lease_agenda && !errors.default_lease_agenda}
                isInvalid={touched.default_lease_agenda && !!errors.default_lease_agenda}
              />
              <Form.Control.Feedback type="invalid">{errors.default_lease_agenda}</Form.Control.Feedback>
            </Form.Group>
          </div>
        </Col>
        <Col xxl={5} md={6}>
          <Form.Group controlId="DefaultLeaseFormAttachment">
...
export default ProviderHOC(DefaultLeaseModal);
