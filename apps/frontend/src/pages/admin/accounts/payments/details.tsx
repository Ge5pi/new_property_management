...
import { useGetInvoiceByIdQuery } from 'services/api/invoices';
import {
...
const Details = () => {
  ...
  const payment = useGetPaymentsByIdQuery(getValidID(payment_id));
  const invoice = useGetInvoiceByIdQuery(getIDFromObject('invoice', payment.data));
  const bank = useGetBankAccountByIdQuery(getIDFromObject('account', payment.data));

  // update account
  const [
    updatePaymentNotes,
    { isSuccess: isUpdatePaymentNotesSuccess, isError: isUpdatePaymentNotesError, error: paymentNotesUpdateError },
  ] = useUpdatePaymentsMutation();

  useResponse({
    isSuccess: isUpdatePaymentNotesSuccess,
    successTitle: 'Note has been successfully updated!',
    isError: isUpdatePaymentNotesError,
    error: paymentNotesUpdateError,
  });

  const handleNoteSubmit = async (value: string) => {
    if (!isNaN(Number(payment_id)) && Number(payment_id) > 0) {
      updatePaymentNotes({ notes: value, id: Number(payment_id) });
    }
  };
                      ...
                      <div className="text-center">
                        <p className="fs-5 fw-bold m-0 text-primary">
                          <span
                            className={clsx(
                              {
                                '-ive': isNegativeNumber(data.amount),
                              },
                              'price-symbol'
                            )}
                          >
